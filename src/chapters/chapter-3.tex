...
\section{Alat-Alat yang Digunakan}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{resources/3-rasaNLU_train.png}
...
\subsubsection{Menjalankan Agen Dialog}
...
spaCy adalah sebuah \textit{open-source library} Python digunakan untuk melakukan pengolahan teks, khususnya NLP, dalam tingkat industri [HON17]. spaCy mendukung banyak bahasa selain MITIE, dan lebih mudah untuk menambahkan bahasa baru, cukup dengan menyediakan model bahasa yang dibutuhkan ke dalam spaCy. Versi spaCy yang digunakan adalah versi 2.0.11.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{resources/3-spacy_arch.png}
...
Rasa NLU menggunakan \textit{library} Python yaitu sklearn-crfsuite. \textit{Library} ini merupakan pembungkus \textit{library} python-crfsuite yang mana menyediakan \textit{estimator} yang cocok dengan scikit-learn \parencite{sklearncrf}. Hal tersebut memudahkan penggunaan model CRF untuk pelatihan dan menyimpan model hasil latihan.

\subsection{Klasifikasi Maksud kalimat dengan \textit{Support Vector Machine}}
...
Rasa NLU menggunakan SVM untuk melakukan klasifikasi maksud kalimat dari scikit-learn. Dengan jumlah maksud kalimat yang berjumlah lebih dari dua, maka scikit-learn akan menggunakan metode-metode untuk mengakali SVM sehingga dapat digunakan untuk kasus melakukan klasifikasi lebih dari dua kelas.

\section{Analisis Kekurangan Sistem}
...
\subsection{Model yang Digunakan Tidak Cukup}
...
\subsection{Perbaikan Algoritma yang Digunakan}

\subsection{Kebutuhan Ekstraksi Fitur untuk CRF}

\subsection{Kecocokan SVM untuk Klasifikasi Lebih dari Dua Kelas}
...
Kelemahan yang dimiliki oleh skema one-versus-one adalah jumlah klasifikasi yang dilakukan, yaitu (jumlah kelas * (jumlah kelas – 1) / 2) klasifikasi, membuat skema ini memiliki kompleksitas O(jumlah kelas \^ 2). Dengan jumlah klasifikasi yang banyak, SVM dirasa tidak efisien untuk melakukan klasifikasi dengan jumlah kelas yang banyak. Oleh karena itu, metode klasifikasi yang tepat digunakan untuk mengatasi masalah jumlah kelas lebih dari dua tersebut adalah neural network.

Keluaran dari neural network dapat diatur sesuai dengan jumlah kelas yang tersedia pada data latihan. Fleksibilitas yang dimiliki oleh neural network membuat metode ini dapat melakukan satu model dan satu kali latihan klasifikasi untuk dapat menghasilkan model yang bagus untuk digunakan.